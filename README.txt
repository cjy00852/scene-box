...
이름 정리와 Drive 복구
- 메인 상단 이름 바꾸기: 날짜_행사_멤버_번호 형식, 행사 없으면 일반
- 전체 적용 시 기존 Drive 파일도 같은 이름으로 변경 (연결이 유효할 때 처리)
- 동기화 창 아래 업로드 중인 파일과 최근 완료 20개 표시
- 앱을 열어둔 동안 약 1분마다 SCENE BOX 폴더 확인: 휴지통이면 복원, 영구 삭제면 로컬 자료로 재생성

처음 사용 안내
- 자료가 없는 첫 실행에서 8단계 튜토리얼 표시
- 상단 사용법 버튼으로 다시 보기, 닫기/이전/다음 지원
- 로컬 검증: npm run test:tutorial
