...
async function main(){
 ...
 const pair=page.locator('.category-pair');assert.equal(await pair.locator('[data-m="category:legacy-membership"]').count(),1);
 ...
 await page.click('#selectAllMedia');assert.equal(await page.locator('#selectionStatus').innerText(),'3개 선택');
